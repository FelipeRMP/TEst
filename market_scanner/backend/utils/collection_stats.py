...
def load_collection_stats() -> dict[str, object]:
    ...
    recent_price_snapshot_count_24h = sqlite_summary.get("recent_price_snapshot_count_24h", price_summary["count_24h"])

    latest_candidates = [
        timestamp
        for timestamp in (
            sqlite_summary.get("latest_scan_timestamp"),
    ...
    return {
        ...
        "simulated_realized_pnl": sqlite_summary.get("simulator_total_pnl", 0.0),
        ...
        "win_rate": sqlite_summary.get("simulator_win_rate"),
        "recent_scan_activity": sqlite_summary.get("recent_scan_activity", _build_recent_activity(signal_rows, price_rows)),
        "total_scan_batches": sqlite_summary.get("total_scan_batches", 0),
        "latest_scan_batch_timestamp": sqlite_summary.get("latest_scan_timestamp"),
        "latest_scan_duration_seconds": sqlite_summary.get("latest_scan_duration_seconds", 0.0),
        "unique_markets_scanned": sqlite_summary.get("unique_markets_scanned", 0),
        "unique_events_signaled": sqlite_summary.get("unique_events_signaled", 0),
        "malformed_id_count": sqlite_summary.get("malformed_id_count", 0),
        "top_repeated_signal_families": sqlite_summary.get("top_repeated_signal_families", []),
    }
...
def _sqlite_summary() -> dict[str, Any]:
    ...
    try:
        with sqlite3.connect(SQLITE_PATH) as connection:
            ...
            batch_summary = _query_scan_batches(connection)
...
def _query_signal_table(connection: sqlite3.Connection) -> dict[str, Any]:
    ...
    row = connection.execute(
        ...
        SELECT
            ...
            SUM(CASE WHEN market_id LIKE '%:YES:YES' OR market_id LIKE '%:NO:NO' THEN 1 ELSE 0 END)
        FROM signals
        """,
        (since,),
    ).fetchone()
    ...
    repeated = connection.execute(
        ...
        SELECT
            ...
            MAX(timestamp)
        FROM signals
        GROUP BY COALESCE(signal_family, '')
        HAVING COUNT(*) > 1
    ...
    return {
        ...
        "unique_events_signaled": int(row[5] or 0),
        "malformed_id_count": int(row[6] or 0),
        "top_repeated_signal_families": [
            {
            ...
            for family, event_title, count, last_seen in repeated
            if family
        ],
    }
...
def _query_price_table(connection: sqlite3.Connection) -> dict[str, Any]:
    ...
    row = connection.execute(
        ...
        SELECT
            COUNT(*),
            MAX(COALESCE(scan_timestamp, timestamp)),
            SUM(CASE WHEN COALESCE(scan_timestamp, timestamp) >= ? THEN 1 ELSE 0 END),
            COUNT(DISTINCT market_id)
        FROM price_history
        """,
        (since,),
    ...
    return {
        ...
        "unique_markets_scanned": int(row[3] or 0),
    }


def _query_scan_batches(connection: sqlite3.Connection) -> dict[str, Any]:
    if not _table_exists(connection, "scan_batches"):
        return {}
    ...
    row = connection.execute(
        ...
        SELECT
            COUNT(*),
            MAX(COALESCE(scan_timestamp, started_at)),
            COALESCE(
                (
                    SELECT duration_seconds
                    FROM scan_batches
                    ORDER BY COALESCE(scan_timestamp, started_at) DESC
                    LIMIT 1
                ),
                0
            )
    ...
    activity_rows = connection.execute(
        ...
        SELECT
            scan_id,
            COALESCE(scan_timestamp, started_at),
            COALESCE(emitted_signal_count, 0),
            COALESCE(price_snapshot_count, 0),
    ...
    return {
        "total_scan_batches": int(row[0] or 0),
        "latest_scan_timestamp": _parse_timestamp(row[1]),
        "latest_scan_duration_seconds": float(row[2] or 0.0),
        "recent_scan_activity": [
            {
                "scan_id": scan_id,
                "timestamp": _parse_timestamp(timestamp),
                "signal_count": int(signal_count or 0),
                "price_snapshot_count": int(price_snapshot_count or 0),
                "status": status,
                "duration_seconds": float(duration_seconds or 0.0),
            }
            for scan_id, timestamp, signal_count, price_snapshot_count, status, duration_seconds in activity_rows
            if timestamp
        ],
    }
...
def _query_simulated_trades(connection: sqlite3.Connection) -> dict[str, Any]:
    ...
    latest_run_id = None
    if _column_exists(connection, "simulated_trades", "run_id"):
        latest = connection.execute(
            "SELECT run_id FROM simulated_trades ORDER BY COALESCE(run_timestamp, entered_at) DESC LIMIT 1"
        ).fetchone()
...
def _build_recent_activity(
    ...
    for row in signal_rows:
        scan_id = row.get("scan_id") or row.get("timestamp") or ""
        timestamp = _parse_timestamp(row.get("scan_timestamp") or row.get("timestamp"))
        if not scan_id or timestamp is None:
            continue
        activity = grouped.setdefault(scan_id, ScanActivity(scan_id=scan_id, timestamp=timestamp))
